...
public:
    ...
    Vec(int size): n(0), body(0)
    {
        create(size);
    }

    Vec(const Vec &src): n(0), body(0)
    {
        copy(src);
    }
    //!< copy constructor
    ...
    {
        return (this==&src? *this: copy(src));    //!< dst = src
    }
    ...
    friend Vec operator*(const Vec &a, double c);
...
public:
    ...
    Matrix(int rows, int cols): nrows(0), ncols(0), body(0)
    {
        create(rows,cols);
    }
    Matrix(const Matrix &src): nrows(0), ncols(0), body(0)
    {
        copy(src);    //!< copy	constructor
    }
    ...
    int cols()const
    {
        return ncols;    //!< returns the number of columns
    }
    int rows()const
    {
        return nrows;    //!< returns the number of rows
    }
